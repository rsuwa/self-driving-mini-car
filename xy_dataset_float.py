...
class XYDataset_Float(torch.utils.data.Dataset):
    def __init__(self, directory, categories, transform=None, random_hflip=False):
        super(XYDataset_Float, self).__init__()
    ...
    def __getitem__(self, idx):
        ...
        # return image, ann['category_index'], torch.Tensor([x])
    ...
    def refresh(self):
        ...
        for category in self.categories:
            ...
            for image_path in glob.glob(
                os.path.join(self.directory, category, "*.jpg")
            ):
    ...
    def save_entry(self, category, image, x, y):
        ...
        filename = "%f_%f_%s.jpg" % (x, y, str(uuid.uuid1()))
